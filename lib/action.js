...
import { filterTagsByBranchAncestry, getBranchFromRef, isPr, getCommits, getLatestPrereleaseTag, getLatestTag, getValidTags, mapCustomReleaseRules, mergeWithDefaultChangelogRules, } from './utils.js';
import { createTag, listTags } from './github.js';
const TAG_CONTEXTS = ['repo', 'branch'];
const BRANCH_HISTORIES = ['compare', 'last', 'full'];
export default async function main() {
    ...
    if (tagMessage && !createAnnotatedTag) {
        core.warning('tag_message was set but create_annotated_tag is false; tag_message is ignored for lightweight tags.');
    }
    ...
    let tagContext = 'repo';
    if (TAG_CONTEXTS.includes(tagContextInput)) {
        tagContext = tagContextInput;
    }
    else {
        core.warning(`${tagContextInput} is not a valid tag_context. Falling back to repo.`);
    }
    const branchHistoryInput = core.getInput('branch_history') || 'compare';
    let branchHistory = 'compare';
    if (BRANCH_HISTORIES.includes(branchHistoryInput)) {
        branchHistory = branchHistoryInput;
    }
    else {
        core.warning(`${branchHistoryInput} is not a valid branch_history. Falling back to compare.`);
    }
    ...
    const validTags = tagContext === 'branch'
        ? await filterTagsByBranchAncestry(allValidTags, commitRef, prefixRegex, {
            gitCwd,
        })
        : allValidTags;
    const latestTag = getLatestTag(validTags, prefixRegex, tagPrefix, initialVersion);
    const latestPrereleaseTag = getLatestPrereleaseTag(validTags, identifier, prefixRegex);
    let commits;
    let newVersion;
    if (customTag) {
        commits = await getCommits(latestTag.commit.sha, commitRef, commitRangeOptions);
        ...
        commits = await getCommits(previousTag.commit.sha, commitRef, commitRangeOptions);
        core.debug('We found ' + commits.length + ' commits to consider!');
        if (scopes.length) {
            const commitParser = new CommitParser();
            const isInScope = (scope) => scopes.split(',').some((includedScope) => scope.match(includedScope));
    ...
    await createTag(newTag, createAnnotatedTag, tagExists, commitRef, pushTag, tagMessage);
