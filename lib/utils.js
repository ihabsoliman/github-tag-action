...
import { getCommitRange, getCompareStatus, isShallowRepository, listMergedTags, } from './github.js';
...
export async function getValidTags(tags, prefixRegex) {
...
export async function getCommits(baseRef, headRef, options = {}) {
    let commits;
...
function getClosedPRCommits() {
    ...
    if (!('pull_request' in context.payload) &&
        ...
        core.debug('After processing we are going to present ' +
            commits.length +
            ' commits!');
    }
    return commits;
...
export function getLatestTag(tags, prefixRegex, tagPrefix, initialVersion = '0.0.0') {
 ...
 */
export async function filterTagsByBranchAncestry(tags, sha, prefixRegex, options = {}) {
    if (tags.length === 0) {
        return tags;
    }
    if (!(await isShallowRepository(options.gitCwd))) {
        try {
            ...
            if (mergedTagNames.length > 0) {
                core.info('tag_context: branch - using local git ancestry (git tag --list --merged).');
                const mergedTagNameSet = new Set(mergedTagNames);
                return tags.filter((tag) => mergedTagNameSet.has(tag.name));
            }
            core.info('tag_context: branch - local git reported no merged tags; falling back to the API scan (tags may never have been fetched locally).');
        }
        catch (error) {
            core.warning(`tag_context: branch - local git ancestry check failed: ${error?.message}. Falling back to the API scan.`);
        }
    }
    else {
        core.info('tag_context: branch - checkout is shallow; using the API scan instead of local git.');
    }
    ...
    for (const tag of tags) {
        if (scans >= ANCESTRY_API_SCAN_LIMIT) {
            core.warning(`tag_context: branch - reached the API scan limit (${ANCESTRY_API_SCAN_LIMIT}); remaining tags were not checked.`);
            break;
        }
        scans++;
        let status;
        try {
            ...
            core.warning(`tag_context: branch - ancestry check failed for tag ${tag.name}: ${error?.message}. Skipping.`);
            continue;
        }
